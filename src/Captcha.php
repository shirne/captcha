...
class Captcha
{
     ...
     */
    public function __construct($config = [], $store=null)
    {
        $this->config = array_merge($this->config, $config);
        $this->store = $store==null ? Session::instance() : $store;
    }
    ...
    public function check($code, $id = '')
    {
        ...
        if ($this->authcode(strtoupper($code)) == $secode['verify_code']) {
            $this->reset && $this->deleteStore($key);
            return true;
        }
    ...
    private function deleteStore($key){
        if($this->store instanceof ThinkSession){
            return $this->store->delete($key, '');
        }else if($this->store instanceof Cache){
            return $this->store->rm($key);
        }
        return false;
    }
